...
public class BossBarRenderEvent {
    ...
    private static final int BAR_WIDTH = (int)(230);
    private static final int BAR_HEIGHT = (int)(29);
    ...
    public static void onRenderBossBar(CustomizeGuiOverlayEvent.BossEventProgress event) {
        if (event.getBossEvent().getName().getString().equals(Component.translatable("event.curse_of_desert.name").getString())) {
            ...
            // 渲染波次信息
            CurseOfDesertEvent activeEvent = CurseOfDesertEventHandler.getCurrentEvent();
            if (activeEvent != null) {
                int currentWave = activeEvent.getCurrentWave();
                int totalWaves = activeEvent.getTotalWaves();
                String waveText = currentWave + "/" + totalWaves;
                
                Font font = Minecraft.getInstance().font;
                int textWidth = font.width(waveText);
                int textX = screenWidth / 2 - textWidth / 2 + 2;
                int textY = y - 2;
                
                int sandYellowColor = 0xFFEECBAD;
                guiGraphics.drawString(font, waveText, textX, textY, sandYellowColor);
            }
